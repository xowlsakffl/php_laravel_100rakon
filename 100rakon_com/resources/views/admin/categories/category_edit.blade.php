...
<div class="section_wrap clearfix">
    ...
    <div class="content_wrap">
        ...
        <div class="main_wrap clearfix">
            <form action="{{route('admin.categories.update' ,['pcdx' => $categoryData->pcdx])}}" method="post" class="add_form">
                @csrf
                @method('PUT')
                ...
                <div class="form_add">
                    ...
                    <select name="category" class="form_text">
                        @foreach ($categories as $category)
                            <option value="{{$category->pcdx}}" @if($category->cname === $categoryData->cname) selected @endif>{{$category->cname}}</option>
                        @endforeach
                    </select>
                    @error('category')
                        <div class="invalid-feedback">
