...
<div class="section_wrap clearfix">
    ...
    <div class="content_wrap">
        ...
        <div class="main_wrap clearfix">
            <form action="{{route('admin.outstand-categories.store')}}" method="post" class="add_form">
                @csrf
                <div class="form_add">
                    <label for="">분류명</label>
                    <input type="text" name="cname" class="form_text @error('cname') is-invalid @enderror" value="{{old('cname')}}">
                    @error('cname')
                ...
                <div class="form_add">
                    ...
                    <select name="category" class="form_text">
                            <option value="0">최상위 분류</option>
                        @foreach ($categories as $category)
                            <option value="{{$category->pcdx}}">{{$category->cname}}</option>
                        @endforeach
                    </select>
                    @error('category')
                        <div class="invalid-feedback">
                ...
                <div class="form_btn clearfix">
                    <button type="submit">분류 생성</button>
                </div>
            </form>
@endsection
