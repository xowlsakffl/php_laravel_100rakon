    ...
    <div class="content_wrap content_view">
        ...
        <div class="main_wrap clearfix">
            ...
            <div class="info_box clearfix">
                ...
                @endforeach
<form action="/admin/subscrib-orders/history-create" method="post" class="add_form">
        @csrf
                <div class="history_input">
                    <input type="text" name="content" class="form-control text" placeholder="변동 메모" />
                    <button type="submit">입력</button>
                </div>
<input type="hidden" name="sodx" value="{{ $orderData->sodx }}" />
</form>
            </div>
        </div>
        ...
        <div class="main_wrap clearfix">
            <div class="main_wrap_title clearfix">
                <div class="right_btn">
                    <a href="{{route('admin.subscrib-orders.edit', ['sodx' => $orderData->sodx])}}">주문 수정</a>
                    <form action="{{route('admin.subscrib-orders.destroy', ['sodx' => $orderData->sodx])}}" method="post">
                        @csrf
                        @method('DELETE')
        ...
        <div class="main_wrap clearfix">
            ...
            @foreach ($orderItems as $orderItem)
                ...
                <div class="info_box clearfix" style="border-bottom:none">
                    <dl>
                        <dt>제품명</dt>
                        <dd>{{$orderItem->product->product->name}}</dd>
                    </dl>
                    <dl>
                        <dt>구성</dt>
                        @if($orderItem->product->is_basic == "Y")
                            <dd>필수구성</dd>
                        @else
                            <dd>추가구성</dd>
                        @endif
