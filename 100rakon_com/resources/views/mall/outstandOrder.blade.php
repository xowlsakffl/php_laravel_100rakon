...
<div id="mall_order">
    ...
    <div class="title"><div class="square"></div><div class="text">특별상품 주문하기</div></div>
    <div class="header">
    ...
    <div class="item">
        <div class="item_thumb"><img src="<?=Storage::url('public/'.$item['outstand']->thumbnail->real_name)?>" /></div>
        <div class="item_spec">
            <div class="item_title">{{ $item['outstand']->title }}</div>
            <div class="item_price">
                <div class="discount">{{ number_format($item['outstand']->price) }}</div>
                <div class="won">원</div>
                {{-- <div class="normals">{{ number_format($item['outstand']->price_normal) }}</div> --}}
            </div>
        </div>
    ...
    <input type="hidden" name="osdx[]" value="<?=$item['osdx']?>" />
    ...
    <input type="hidden" name="need_delivery_info[]" value="<?=$item['need_delivery_info']?>" />
    @endforeach
    ...
    @if ($item['outstand']->need_delivery_info == true)
        ...
        if(result.condition == true)
        {
            if(user){
                if(result.order_info.pay_kind == "무통장")
                {
                    ...
                    if(result.condition == true)
                    {
                        location.href = '/myorder-outstand';
                    }
                }else{
                    //토스 결제 호출
                ...
                if(result.order_info.pay_kind == "무통장")
                {
                    ...
                    if(result.condition == true)
                    {
                        location.href = '/myorder-outstand-guest-input';
                    }
                }else{
                    //토스 결제 호출
                    tossPayments.requestPayment(result.order_info.pay_kind, { // 결제 수단 파라미터
