...
@section('content')
<div id="mall_myorder_subscrib">
    <div class="title">
        <div class="square"></div>
        <div class="text">정기배송 주문내역</div>
        <div class="goto_another" onClick="gen.redirect('myorder');">일반주문내역</div>
    </div>
    <div class="header">
        <div class="number">주문번호</div>
        <div class="bar"></div>
        <div class="amount">월 결제금액</div>
        <div class="bar"></div>
    ...
    <div class="items" id="id_items_{{ $key }}">
        ...
        <div class="item">
            <div class="thumbnail"><img src="<?=Storage::url('public/'.$products[$item['soidx']]->thumbnail->real_name)?>" /></div>
            <div class="name">
                @if($sgProducts[$item['soidx']]->is_basic == "Y")
                    [필수]
                @else
                    [추가]
                @endif
                {{ $products[$item['soidx']]->name }}
            </div>
        ...
        <div class="details">
            <div class="showMobile">
                ...
                <div class="text">
                    &middot 상품이름 : {{ $order->good->title }}<br/>
                    &middot 주문번호 : {{ $order->order_number }}<br/>
                    &middot 배송기간 : {{ $order->term }} 개월<br/>
            ...
            <div>
                ...
                <div class="text">
                    @if(($order->receipt_kind == "세금계산서")||($order->receipt_kind == "현금영수증"))
                    ...
                    @elseif($order->receipt_kind == "매출전표")
                        @if(count($order->toss()) > 0)
                            <a href="" onClick="window.open('{{ $order->toss()[0]['card_receiptUrl'] }}', '영수증', '');" class="Navy" target="_blank">{{ $order->receipt_kind }} 출력하기</a>
                        @endif
                    @elseif(($order->receipt_kind == "소득공제")||($order->receipt_kind == "지출증빙"))
                        @if(count($order->toss()) > 0)
                            <a href="" onClick="window.open('{{ $order->toss()[0]['cashRct_receiptUrl'] }}', '영수증', '');" class="Navy" target="_blank">{{ $order->receipt_kind }} 영수증 출력하기</a>
                        @endif
                    @else
