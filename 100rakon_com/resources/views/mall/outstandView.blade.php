...
<div id="mall_view">
    ...
    <div class="explain">
        ...
        <div class="spec">
            <div class="title">{{ $outstand->title }}</div>
            @if ($outstand->price_normal > 0)
            
            <div class="price">
                <div class="discount_normal Montserrat">{{ number_format($outstand->price_normal) }}</div>
                <div class="discount Montserrat">{{ number_format($outstand->price) }}</div>
            ...
            <div class="exp_line">
                ...
                <div class="tail">{{ $outstand->supply }}</div>
            </div>
            <div class="exp_line">
                <div class="head">배송비</div>
                <?php
                $delivery_cost = $outstand->delivery_origin_cost;
                if($delivery_cost == 0)
                {
                    $delivery_cost = '무료';
                }else{
            ...
            {{-- <div class="exp_line">
                ...
                <div class="tail">
                    <select>
                        ...
                        <option value="">{{$outstand->name}}</option>
                    </select>
                </div>
            </div> --}}
            <div class="total_price">
                <span class="txt01 Montserrat">TOTAL PRICE</span>
                <span class="txt02"><div id="id_total_amount" class="Montserrat">{{ number_format($outstand->price) }}</div></span>
            ...
            <div class="buttons" style="justify-content: right">
                ...
                <button class="darkbtn" onClick="goto_order()">
                    <div>
                        <img src="/images/cart_icon.png" alt=""><span>주문하기</span>
                    </div>
                </button>
            </div>
        </div>
...
<!-- 쉐어용 설정 -->
<head>
<meta property="og:title" content="100rakon.com :: {{ $outstand->title }}" />
<meta property="og:description" content="{{ $outstand->title }}" />
...
<script>
let osdx = {{ $outstand->osdx }};    //번호
let price = parseInt({{ $outstand->price }});    //단가
let quantity = 1;    //수량
...
function goto_order()
{
    ...
    {
        document.location.href = "/outstand/direct/" + osdx + "/" + quantity;
    }
}
</script>
</div>
